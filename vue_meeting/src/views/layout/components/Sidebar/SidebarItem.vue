<template>
  <div v-if="!item.hidden&&item.children" class="menu-wrapper">
    ...
    <template v-for="child in item.children" v-if="!child.hidden">
      <a :href="child.path" target="_blank" @click="clickLink(child.path,$event)" :key="child.name">
        <el-menu-item :index="resolvePath(child.path)" >
          <item v-if="child.meta" :icon="child.meta.icon" :title="generateTitle(child.meta.name)" />
        </el-menu-item>
      </a>
    </template>
